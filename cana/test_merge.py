...
def test_merge_identical_elements():
    A = [1, 1, 2, 2]
    merge(A, 0, 2, 4)
    assert A == [1, 1, 2, 2]


def test_merge_with_negatives():
    A = [-3, -1, 0, -2, 2, 4]
    merge(A, 0, 3, 6)
...
def test_merge_internal_segment():
    A = [9, 1, 3, 5, 2, 4, 6, 0]
    merge(A, 1, 4, 7)  # Merge [1,3,5] and [2,4,6]
